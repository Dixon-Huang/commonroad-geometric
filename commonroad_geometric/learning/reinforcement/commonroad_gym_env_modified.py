...
class CommonRoadGymEnv(gymnasium.Env, Generic[T_SimulationOptions]):
    ...
    def step(
        ...
        try:
            ...
            # for _, action_completed in self.ego_vehicle_simulation.step(action):
            for _, action_completed in self.ego_vehicle_simulation.step(info):
    ...
    def set_policy_and_self_reference(self, policy: Any):
        if hasattr(self._ego_vehicle_simulation.control_space, 'set_policy_and_env'):
            self._ego_vehicle_simulation.control_space.set_policy_and_env(policy, self)