...
from commonroad_geometric.common.io_extensions.scenario import (
    backup_scenario,
    get_dynamic_obstacles_at_timesteps,
    get_scenario_timestep_bounds,
)
from commonroad_geometric.common.types import T_CountParam
from commonroad_geometric.simulation.base_simulation import (
    BaseSimulation,
    BaseSimulationOptions,
    SimulationNotYetStartedException,
)
...
class ScenarioSimulation(BaseSimulation):
    def __init__(
        ...
        super().__init__(initial_scenario=initial_scenario, options=options)
        ...
        self._world_state = World.create_from_scenario(self.current_scenario)

        if options.collision_checking:
            self._time_variant_collision_checker = create_collision_checker(
                self.current_scenario
            )
            self._current_collision_checker_time_step = -1
            self._current_collision_checker = None
            self._time_step_to_obstacles = get_dynamic_obstacles_at_timesteps(
                self.current_scenario
            )
    ...
    def collision_checker(self) -> CollisionChecker:
        ...
        if self._current_collision_checker_time_step != self.current_time_step:
            self._current_collision_checker = (
                self._time_variant_collision_checker.time_slice(self.current_time_step)
            )
            self._current_collision_checker_time_step = self.current_time_step
        return self._current_collision_checker

    @property
    def world_state(self) -> Optional[World]:
    ...
    def _spawn_ego_vehicle(self, ego_vehicle: EgoVehicle) -> None:
        ...
        self.world_state.remove_vehicle_states(
            ego_vehicle.as_dynamic_obstacle.initial_state.time_step
        )
        self.world_state.add_controlled_vehicle(
            self.current_scenario, ego_vehicle.as_dynamic_obstacle
        )
        for obstacle in self.current_obstacles:
            if isinstance(
                self.world_state.vehicle_by_id(obstacle.obstacle_id), ControlledVehicle
            ):
                vehicle: ControlledVehicle = self.world_state.vehicle_by_id(
                    obstacle.obstacle_id
                )
                vehicle.add_state(self.get_current_obstacle_state(obstacle))
    ...
    def _step(self, ego_vehicle: Optional[EgoVehicle] = None) -> None:
        ...
        if ego_vehicle is not None:
            ...
            self._time_step_to_obstacles[self.current_time_step].append(
                ego_vehicle.as_dynamic_obstacle
            )
        for obstacle in self.current_obstacles:
            if isinstance(
                self.world_state.vehicle_by_id(obstacle.obstacle_id), ControlledVehicle
    ...
    def _reset(self) -> None:
        ...
        if self._options.backup_current_scenario:
            self._current_scenario = backup_scenario(
                self.initial_scenario
            )  # TODO: Avoid?
        self._world_state = World.create_from_scenario(self._current_scenario)
        self._start()