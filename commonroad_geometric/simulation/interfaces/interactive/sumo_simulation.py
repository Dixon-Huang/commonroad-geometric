...
class SumoSimulation(BaseSimulation):
    def __init__(
        ...
        self._world_state = World.create_from_scenario(self.current_scenario)  # 新增 world_state 属性
    ...
    def _step(
        ...
        logger.info(f"Stepping with ego vehicle: {ego_vehicle is not None}")
        self.spawn_vehicles()
    ...
    def _spawn_ego_vehicle(self, ego_vehicle: EgoVehicle) -> None:
        logger.info(f"Spawning ego vehicle with ID {ego_vehicle.obstacle_id}")
        # HACK to spawn ego vehicle within SUMO.
        # For self_initialize_sumo no planning_problem_set is available.
        # We only have to spawn the ego vehicle once for it to be included in the SUMO simulation
        #
        ...
        # Run next step with ego vehicle, otherwise ego vehicle will not be added to current_scenario in SumoSimulation._step
        # even though it has spawned just now
        self.world_state.remove_vehicle_states(  # 更新 world_state
            ego_vehicle.as_dynamic_obstacle.initial_state.time_step
        )
        self.world_state.add_controlled_vehicle(  # 更新 world_state
    ...
    def _reset(self) -> None:
        ...
        self._world_state = World.create_from_scenario(self._current_scenario)  # 重新初始化 world_state
    ...
    def _initialize_sumo(self) -> None:
        ...
        self._sumo_sim.initialize(
            ...
            # This will spawn ego vehicles for every planning problem in the planning problem set
