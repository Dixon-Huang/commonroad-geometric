...
class BaseSimulation(LaneletNetworkInfo, Renderable, Iterator[Tuple[int, Scenario]], Generic[T_SimulationOptions], StringResolverMixin):
    ...
    def __next__(self) -> Tuple[int, Scenario]:
        ...
        if self._current_run is None:
            ...
            self._current_run = iter(self.iter_time_steps)
        try:
            self._current_time_step = next(self._current_run) + 1 # Change
    ...
    # Change
    def jump_to_time_step(self, time_step: int) -> None:
        """
        在不模拟中间步骤的情况下，将仿真跳转到指定的时间步。
        """
        if self.initial_time_step <= time_step <= self.final_time_step:
            # 更新当前时间步
            ...
            raise ValueError(f"时间步 {time_step} 超出范围")