...
class EgoVehicle(_BaseVehicle):
    ...
    def set_state(
        ...
        self._dynamic_obstacle.prediction.trajectory.state_list.append(state)
        self._dynamic_obstacle.prediction.trajectory.final_state.time_step = state.time_step
        self.update_collision_object()
        for key, aggregator in self._aggregate_state_statistics.items():
            if hasattr(state, key):
                aggregator.update(getattr(state, key))
        return state
    ...
    def set_next_state(
        ...
        current_states = list(self._dynamic_obstacle.prediction.trajectory.state_list)
        current_states.append(next_state)
        self._dynamic_obstacle._prediction._trajectory._state_list = current_states
    ...
    # Change
    def set_next_state_without_collision_update(
        self,
        next_state: State
    ) -> State:
        assert self._dynamic_obstacle is not None
    ...
    # Change
    def reset_state_list(self, indices=None, time_duration=None):
        assert self._dynamic_obstacle is not None
        if indices is not None:
            self._dynamic_obstacle.prediction.trajectory._state_list = self._dynamic_obstacle.prediction.trajectory._state_list[
                                                                       :-(len(indices))]
        if time_duration is not None:
            ...
            self._dynamic_obstacle.prediction.trajectory._state_list = self._dynamic_obstacle.prediction.trajectory._state_list[
                                                                       :-21]
        self._dynamic_obstacle.prediction.trajectory.final_state.time_step = self._dynamic_obstacle.prediction.trajectory._state_list[-1].time_step
        # self.update_collision_object()
        for state in self._dynamic_obstacle.prediction.trajectory._state_list:
            for key, aggregator in self._aggregate_state_statistics.items():
                aggregator.reset()
                if hasattr(state, key):
                    aggregator.update(getattr(state, key))
