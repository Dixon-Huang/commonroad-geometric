...
class EgoVehicleSimulation(Renderable, AutoReprMixin):
    ...
    def step(self, action: np.ndarray) -> Iterable[Tuple[int, Scenario]]:  # type: ignore
        ...
        try:
            ...
            # for action_completed in self.control_space.step(
            #         ego_vehicle_simulation=self,
            #         action=action
            # ):
            #     if not action_completed:
            #         yield 0, False
            #     yield next(self.simulation), action_completed
    ...
    def restore_state(self, saved_state, indices=None):
        """恢复到保存的状态"""
        # 重置缓存
        self._reset_cache()

        if indices is not None:
            self._ego_vehicle.reset_state_list(indices)
        else:
            self._ego_vehicle.reset_state_list()
        self.simulation.close()
        self.simulation.start()
        self._simulation = self.simulation(
            from_time_step=saved_state['current_time_step'],
            ego_vehicle=self.ego_vehicle
        )
        self._simulation.lifecycle.run_ego()
        self._traffic_extractor._simulation = self._simulation

    def restore_state_without_indices(self, saved_state):
        """恢复到保存的状态"""
        # 重置缓存
        self._reset_cache()
