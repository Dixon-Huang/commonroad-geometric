...
from commonroad_geometric.dataset.scenario.preprocessing.preprocessors.implementations import MergeLaneletsPreprocessor
...
def generate_lanelet_graph_conversion_plot():
    ...
    scenario_iterator = ScenarioIterator(
        Path(INPUT_SCENARIO),
        preprocessor=MergeLaneletsPreprocessor() >> chain_preprocessors(*filters),
    )
    print(f"Found {len(scenario_iterator.scenario_paths)} scenario files")

    for scenario_bundle in scenario_iterator:
        print(f"Processing {scenario_bundle.scenario_path}")
        for cls, show_waypoints, show_edge_weights, show_edge_angles, show_edge_labels, show_node_labels, conversion_options, conversion_summary, output_dir in configurations:
            def plot_graph_wrapper():
                ...
                graph: BaseRoadNetworkGraph = cls.from_scenario(
                    scenario_bundle.input_scenario, **conversion_options)  # type: ignore
                ax = None
