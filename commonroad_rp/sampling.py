                ...
                if trajectory_long.coeffs is not None:
                    ...
                    for d in self.samples_d.samples_at_level(level_sampling).union({x_0_lat[0]}):
                        ...
                        if trajectory_lat.coeffs is not None:
                            ...
                            # Change
                            # self.visualize_trajectory(trajectory_long, trajectory_lat)

        return list_trajectories

    # Change
    def visualize_trajectory(self, trajectory_long, trajectory_lat):
        # 可视化代码，例如绘制轨迹图
        import matplotlib.pyplot as plt

        # 假设 trajectory_long 和 trajectory_lat 都有 evaluate_state_at_tau 方法可以返回轨迹点
        t_samples = np.linspace(0, self.horizon, int(self.horizon / self.dt))
        lon_points = [trajectory_long.evaluate_state_at_tau(t) for t in t_samples]
        lat_points = [trajectory_lat.evaluate_state_at_tau(t) for t in t_samples]

        lon_x = [point[0] for point in lon_points]
        lon_y = [point[1] for point in lon_points]
        lat_x = [point[0] for point in lat_points]
        lat_y = [point[1] for point in lat_points]

        plt.figure()
        plt.plot(lon_x, lon_y, label='Longitudinal Trajectory')
        plt.plot(lat_x, lat_y, label='Lateral Trajectory')
        plt.legend()
        plt.xlabel('x')
        plt.ylabel('y')
