...
_dict_traj_status_to_color = {
    FeasibilityStatus.FEASIBLE.name: 'green', # Change: 'blue' to 'green'
    FeasibilityStatus.INFEASIBLE_KINEMATIC.name: 'grey', # Change: 'blue' to 'grey'
    FeasibilityStatus.INFEASIBLE_COLLISION.name: 'red' # Change: 'blue' to 'red
}
...
def make_gif(config: ReactivePlannerConfiguration, time_steps: Union[range, List[int]], duration: float = 0.1):
    ...
    if not config.debug.save_plots:
        ...
        print(f"...GIF created: {os.path.join(config.general.path_output, str(scenario_id) + '.gif')}")
