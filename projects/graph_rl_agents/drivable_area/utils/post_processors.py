...
from projects.geometric_models.drivable_area.models.scenario_models import ScenarioDrivableAreaModel, plot_all_vehicles
...
class EncodingPostProcessor(BaseDataPostprocessor):
    ...
    def __init__(
        ...
        self._show_plot = True and sys.gettrace() is not None
        if self._show_plot:
            plt.ion()
    ...
    def __call__(
        ...
        for data in samples:
            ...
            if self._enable_decoding:
                predictions = self._model.decode(cr_data=data, x_vehicle=encoding)
                data.vehicle.prediction = predictions
