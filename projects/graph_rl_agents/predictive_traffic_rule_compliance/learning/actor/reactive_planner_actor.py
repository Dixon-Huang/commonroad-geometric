...
from projects.graph_rl_agents.predictive_traffic_rule_compliance.reactive_planner.reactive_planner import \
    ReactivePlanner
from projects.graph_rl_agents.predictive_traffic_rule_compliance.reactive_planner.cost_function.robustness_cost_function_simplified import \
    RobustnessCostFunctionSimplified
from projects.graph_rl_agents.predictive_traffic_rule_compliance.reactive_planner.cost_function.robustness_cost_function import \
    RobustnessCostFunction
from projects.graph_rl_agents.predictive_traffic_rule_compliance.reactive_planner.cost_function.robustness_cost_function_fast import \
    RobustnessCostFunctionFast
from projects.graph_rl_agents.predictive_traffic_rule_compliance.reactive_planner.cost_function.baseline_cost_function import \
    ...
    def step(
        ...
        try:
            ...
            if plan_new_trajectory:
                ...
                #                                                       ego_vehicle_simulation=ego_vehicle_simulation_deepcopy,
                #                                                       ))

                self.planner.set_cost_function(BaselineCostFunction(self.planner.desired_speed,
                                                                      desired_d=0.0,
                                                                      desired_s=self.planner.desired_lon_position,
                                                                      ego_vehicle_simulation=ego_vehicle_simulation_deepcopy,
                                                                      ))

                # self.planner.set_cost_function(RobustnessCostFunctionFast(self.planner.desired_speed,
                #                                                           desired_d=0.0,
                #                                                           desired_s=self.planner.desired_lon_position,
                #                                                           value_function=value_function,
                #                                                           ego_vehicle_simulation=ego_vehicle_simulation_deepcopy,
                ...
                ego_vehicle = self.planner.convert_state_list_to_commonroad_object(
                    [self.optimal_trajectory[0].state_list[1]])
                ...
                # record state and input
                ego_vehicle = self.planner.convert_state_list_to_commonroad_object(
                    [self.optimal_trajectory[0].state_list[1 + temp]])
                ...
                self.planner.reset(initial_state_cart=self.planner.record_state_list[-1],
                                   initial_state_curv=(
                                   self.optimal_trajectory[2][1 + temp], self.optimal_trajectory[3][1 + temp]),
                                   collision_checker=self.planner.collision_checker,
                                   coordinate_system=self.planner.coordinate_system)
            ...
            array = [np.array(
                [position_x, position_y, velocity, orientation, yaw_rate, acceleration, time_step, slip_angle])]
