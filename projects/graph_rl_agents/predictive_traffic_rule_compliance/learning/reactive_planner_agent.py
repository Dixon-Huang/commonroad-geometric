...
from projects.graph_rl_agents.predictive_traffic_rule_compliance.learning.actor.reactive_planner_actor_simplified import ReactivePlannerActorSimplified
...
class ReactivePlannerAgent(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        if env.num_envs > 1:
            ...
            # self.reactive_actor = ReactivePlannerActorSimplified(actor_options)
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            rp_actions = self.reactive_actor.step(self._ego_vehicle_simulations, self.policy)

            if rp_actions is None:
                logger.warning("Planning failed. Resetting the environment.")
                new_obs = env.reset()
                self._last_obs = new_obs
                self._ego_vehicle_simulations = [info.get('ego_vehicle_simulation') for info in env.reset_infos]
                continue

            # 在主进程中打印每个环境的UUID和动作
            for i, (uuid, action) in enumerate(zip([ego.uuid for ego in self._ego_vehicle_simulations], rp_actions)):
                print(f"Main process - Env {i}, UUID: {uuid}, Action: {action}")
            print("=" * 200)

            # logging.info([ego_vehicle_simulations.uuid for ego_vehicle_simulations in self._ego_vehicle_simulations])
            # logging.info(rp_actions)

            new_obs, rewards, dones, infos = env.step(rp_actions)
