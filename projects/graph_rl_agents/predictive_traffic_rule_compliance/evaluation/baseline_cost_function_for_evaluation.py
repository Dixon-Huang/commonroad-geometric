...
class BaselineCostFunction(CostFunction):
    ...
    def __init__(self, desired_speed: Optional[float] = None,
        ...
        self.ego_dynamic_obstacle = ego_dynamic_obstacle
    ...
    def evaluate(self, trajectory: commonroad_rp.trajectories.TrajectorySample):
        ...
        if self.world_state is not None:
            self.controlled_vehicle: ControlledVehicle = self.world_state.vehicle_by_id(
                self.obstacle_id
            )
            self.controlled_vehicle.states_cr = dict(list(self.controlled_vehicle.states_cr.items())[:1])
            # Compute rule robustness at specific points along the trajectory
            robustness_list, time_duration = self._calculate_robustness(trajectory)
    ...
    def _calculate_robustness(self, trajectory: commonroad_rp.trajectories.TrajectorySample):
        ...
        ego_vehicle_world_state = self.world_state.vehicle_by_id(self.obstacle_id)
        rule_evaluators = []
        ...
        for evaluator in rule_evaluators:
            evaluator._last_evaluation_time_step = self.initial_time_step
        ...
        for i in range(len(trajectory.cartesian.x) - 1):
            if i in indices_set:
                ...
                position_center = position_rear + self.vehicle.parameters.b * np.array(
                ...
                self.controlled_vehicle.add_state(state)
        ...
        return robustness_list, len(trajectory.cartesian.x)
