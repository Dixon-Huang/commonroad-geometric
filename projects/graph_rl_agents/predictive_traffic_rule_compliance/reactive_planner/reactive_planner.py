...
class ReactivePlanner(object):
    ...
    def reset(self, config: ReactivePlannerConfiguration = None,
        ...
        # Change: Visualize collision checker
        # from commonroad_rp.cost_function.visualization import visualize_collision_checker
        # visualize_collision_checker(self.config.scenario, self._cc)
    ...
    def set_collision_checker(self, scenario: Scenario = None, collision_checker: pycrcc.CollisionChecker = None,
        ...
        if collision_checker is None:
            ...
            for co in scenario.dynamic_obstacles:
                ...
                # # Change: Skip obstacles with obstacle_id == -1
                # if co.obstacle_id == -1:
                #     continue
    ...
    def _compute_initial_states(self, x_0: ReactivePlannerState) -> (np.ndarray, np.ndarray):
        ...
        try:
            ...
            # Change
            logger.critical(f"初始状态 {x_0.position} 超出投影域。")
        ...
        s_acceleration = x_0.acceleration if x_0.acceleration is not None else 0  # Change: handle None case
        ...
        for trajectory in feasible_trajectories:
            ...
            if self.config.planning.continuous_collision_check and not collide:
                ...
                [ego_tvo.append_obstacle(
                    # Change
                    # pycrcc.RectOBB(half_length, half_width, theta[i], pos1[i], pos2[i])) for i in range(len(pos1))]
                    pycrcc.RectOBB(half_length, half_width, theta[i], pos1[i], pos2[i])) for i in range(round(len(pos1) / 3))] # Change
                ego_tvo, err = trajectory_preprocess_obb_sum(ego_tvo)
                if self._cc.collide(ego_tvo):
                    self._infeasible_count_collision += 1
                    collide = True
    ...
    @property
    def desired_lon_position(self):
        return self._desired_lon_position

    def _calculate_costs_parallel(self, trajectories, queue):
        costs = []
        for trajectory in trajectories:
            cost = self.cost_function.evaluate(trajectory)
