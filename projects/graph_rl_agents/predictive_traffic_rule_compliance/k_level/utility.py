...
def smooth_cv(cv_init):
    # 确保cv_init是numpy数组
    if not isinstance(cv_init, np.ndarray):
        cv_init = np.array(cv_init)
    ...
    # 检查点的数量
    num_points = len(cv)
    if num_points < 4:  # 3阶样条至少需要4个点
        # 如果点太少，先进行线性插值增加点数
        s_temp = np.linspace(0, 1, num_points)
        s_dense = np.linspace(0, 1, 10)
        dense_x = np.interp(s_dense, s_temp, list_x)
        dense_y = np.interp(s_dense, s_temp, list_y)
        cv = np.column_stack((dense_x, dense_y))
        list_x = dense_x
        list_y = dense_y
    ...
    try:
        # 动态选择样条阶数和平滑参数
        k = min(3, len(cv) - 1)  # 确保k小于点数
        smooth_factor = 0.1 * len(cv)  # 根据点数调整平滑因子

        # 进行样条插值
        bspl_x = splrep(s_cv, list_x, k=k, s=smooth_factor)
        bspl_y = splrep(s_cv, list_y, k=k, s=smooth_factor)
        ...
        # print(f"Spline order k: {k}")
        # print(f"Smoothing factor: {smooth_factor}")

        # 可选：添加可视化
        # plt.figure(figsize=(10,6))
        # plt.plot(cv_init[:, 0], cv_init[:, 1], 'rx', label='Original')
        # plt.plot(new_cv[:, 0], new_cv[:, 1], 'b-', label='Smoothed')
        # plt.legend()
        # plt.grid(True)
        # plt.show()
        ...
        print(f"Falling back to original curve")
        # 如果插值失败，返回原始曲线的密集采样
        s_dense = np.linspace(0, max(s_cv), 1000)
        x_dense = np.interp(s_dense, s_cv, list_x)
        y_dense = np.interp(s_dense, s_cv, list_y)
