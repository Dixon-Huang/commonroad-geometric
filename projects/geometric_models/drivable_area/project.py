...
from projects.geometric_models.drivable_area.utils.log_drivable_area_callback import LogDrivableAreaWandb, LogVelocityFlowWandb
...
from projects.geometric_models.drivable_area.utils.vectorized_occupancy_post_processor import compute_vectorized_occupancy

def save_compute_occupancy_vectorized(data: CommonRoadData):
    ...
    # axs[0].imshow(data.v.drivable_area[0, ...].detach().cpu().numpy())
    # axs[1].imshow(occupancy[0, ...].detach().cpu().numpy())
    # plt.show()
...
class ComputeOccupancyTransformation(BaseDataTransformation):
    ...
    @staticmethod
    def transform(scenario_index: int, sample_index: int, data: CommonRoadData):
        save_compute_occupancy_vectorized(data=data)  
        yield data

    def _transform_dataset(self, dataset: CommonRoadDataset) -> CommonRoadDataset:
        dataset_transformation(
            dataset=dataset,
            transform=ComputeOccupancyTransformation.transform,
            num_workers=self.num_workers
        )
        return dataset

    def _transform_data(self, data: CommonRoadData) -> CommonRoadData:
        return ComputeOccupancyTransformation.transform(0, 0, data)
...
class DrivableAreaProject(BaseGeometricProject):
    ...
    def configure_training_callbacks(
        ...
        callbacks_computers = CallbackComputersContainer(
            ...
            validation_step_callbacks=CallbackComputerContainerService([
                ...
                LogVelocityFlowWandb(wandb_service=wandb_service),
                LogDrivableAreaWandb(wandb_service=wandb_service, target_attribute="polar_occupancy"),
                LogVelocityFlowWandb(wandb_service=wandb_service, occupancy_attribute="polar_occupancy", target_attribute="polar_occupancy_flow"),
            ]),
    ...
    @register_run_command
    def plot(self) -> None:
        from projects.geometric_models.drivable_area.models.scenario_models import plot_all_vehicles
