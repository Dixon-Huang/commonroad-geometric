...
from projects.geometric_models.drivable_area.models.decoder.velocity_flow_decoder import VelocityFlowDecoder
...
def plot_vehicle_data(i, axs, data, predictions):
    ...
    axs[1, 0].axis('off')
    axs[1, 1].imshow(reshape_and_visualize_occupancy_flow(predictions['occupancy_flow'][i]))
    ...
    axs[3, 0].axis('off')
    axs[3, 1].imshow(reshape_and_visualize_occupancy_flow(predictions['polar_occupancy_flow'][i]))
...
def plot_all_vehicles(data, predictions, encodings=None, fig=None, axs=None):
    num_nodes = len(data.v.occupancy)  # Assuming occupancy is per node
    ...
    indices = torch.where(data.v.is_ego_mask)[0].tolist() if "is_ego_mask" in data.v and data.v.is_ego_mask.sum() > 0 else [0]
    ...
    if encodings is not None:
        for i in indices:
            ...
            ax.set_title('Encoding - Vehicle {i}')
...
class ScenarioDrivableAreaModel(BaseGeometric):
    ...
    def forward(
        ...
        predictions = self.decode(cr_data, x_vehicle, sampling_weights)
        ...
        if True and sys.gettrace() is not None:
            ...
            if self.__dict__['_plt_counter'] & PLT_FREQ == 0:
                plot_all_vehicles(data=cr_data, predictions=predictions, encodings=x_vehicle)
    ...
    def decode(self, cr_data, x_vehicle, sampling_weights=None) -> dict[str, Tensor]:
        predictions = {}
        for key, decoder in self.decoder_heads.items():
            predictions[key] = decoder(cr_data, x_vehicle, sampling_weights=sampling_weights)
        return predictions
    ...
    def _build(
        ...
        self.decoder_heads = nn.ModuleDict({
            'occupancy': DrivableAreaDecoder(cfg=self.cfg.drivable_area_decoder, target_attribute="occupancy"),
            'occupancy_flow': VelocityFlowDecoder(cfg=self.cfg.drivable_area_decoder, target_attribute="occupancy_flow"),
            'polar_occupancy': DrivableAreaDecoder(cfg=self.cfg.drivable_area_decoder, target_attribute="polar_occupancy"),
            'polar_occupancy_flow': VelocityFlowDecoder(cfg=self.cfg.drivable_area_decoder, target_attribute="polar_occupancy_flow"),
        })
