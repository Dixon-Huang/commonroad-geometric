...
class VelocityFlowDecoder(BaseModel):

    def __init__(
        self,
        cfg: Config,
        target_attribute: str = "velocity_flow"
    ):
        super().__init__()
    ...
    def compute_loss(
        ...
        if isinstance(prediction, tuple):
            ...
            velocity_flow = data.v[self.target_attribute].index_select(
                0, sample_ind) if sample_ind is not None else data.v[self.target_attribute]
        else:
            velocity_flow = data.v[self.target_attribute]

        if only_ego:
            is_ego_mask = data.v.is_ego_mask.bool().squeeze(-1)
            velocity_flow = velocity_flow[is_ego_mask, ...]

        N = prediction.shape[0]
        prediction_size = self.cfg.prediction_size
        velocity_flow = velocity_flow.view(N, prediction_size, prediction_size, 2)
        assert_size(prediction, (N, 2 * prediction_size ** 2))

        vehicle_type_filter = (data.v.type == 0).squeeze(-1)
        if only_ego:
            vehicle_type_filter = torch.ones(N, dtype=torch.bool, device=velocity_flow.device)
        
        
        # Bird's eye view binary drivable area
        true_velocity_flow = velocity_flow.view(N, 2* prediction_size ** 2)
        loss = F.mse_loss(input=prediction[vehicle_type_filter], target=true_velocity_flow[vehicle_type_filter], reduction="mean")
        return loss
